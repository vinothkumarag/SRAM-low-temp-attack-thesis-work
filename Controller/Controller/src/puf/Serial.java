...
public class Serial implements SerialPortEventListener
{
	...
	public Serial(String iname, int irate, int iparity, int idatabits,int istopbits)
	{
		try
		{
			port = null;
			CommPortIdentifier portId = CommPortIdentifier.getPortIdentifier(iname);
			port = (SerialPort) portId.open("serial madness", 100);
			input = port.getInputStream();
			output = port.getOutputStream();
			port.setSerialPortParams(irate, idatabits, istopbits, iparity);
		...
		{
			System.err.println("Serial port "+iname+" already in use. Try quiting any programs that may be using it.");
		}
		catch (Exception e)
		{
			System.err.println("Error opening serial port "+iname+".");
		}

		if (port == null)
		{
			System.err.println("Serial port "+iname+" not found. Did you select the right one from the menu?");
		}
	}
	...
	public static String[] getPorts()
	{
		...
		try
		{
			...
			{
				System.out.println("Could not find any port.");
			}
		}
		...
		String[] result = new String[portNames.size()];
		return portNames.toArray(result);
	}
	...
	public void addListener(IMessageConsumer consumer)
	{
